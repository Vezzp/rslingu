...
% Команда для правильного выбора содержимого морфемы в завимости от значения флага сокрытия
% содержимого морфемы.
\cs_new_protected:Nn \l__rslingu_morpheme_place_content_cs:n {
    \bool_if:nTF { \l__rslingu_morpheme_phantom_bool || \l__rslingu_phantom_bool } {
            \node[rsContentBox] (Baseline) { \phantom{#1} \c__rslingu_vmax_phantom_tl};
...
\ExplSyntaxOn
    \NewDocumentCommand{\rsEnding} { O{} m } {
        \group_begin:
            ...
            \tikz[baseline=(Baseline.base)] {
                \bool_if:nTF { \l__rslingu_phantom_bool } {
                    \node[rsContentBox, outer~sep=1pt] (Baseline) { \phantom{о} \c__rslingu_vmax_phantom_tl};
                    \node at (Baseline.center) {\textbullet};
                } {
                    \node[rsContentBox, outer~sep=1pt] (Baseline) { \ifblank{ #2 }{ \phantom{о} }{ #2 } \c__rslingu_vmax_phantom_tl };
                }
                ...
                in
                    ...
                    (\x1 - .75, \y1 - \c__rslingu_morpheme_base_sign_height_dim) |-
                    (\x2 + .75, \y2 + \c__rslingu_morpheme_sign_y_pad_dim + \c__rslingu_morpheme_sign_height_dim) |-
                    cycle;
            }
